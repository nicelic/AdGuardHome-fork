...
import { splitByNewLine, sortClients, filterOutComments, msToSeconds, msToMinutes, msToHours, } from '../helpers/helpers';
import { BLOCK_ACTIONS, CHECK_TIMEOUT, STATUS_RESPONSE, SETTINGS_NAMES, MANUAL_UPDATE_LINK, DISABLE_PROTECTION_TIMINGS, } from '../helpers/constants';
...
export const checkStatus = async (handleRequestSuccess, handleRequestError, attempts = 60) => {
    let timeout;
    if (attempts === 0) {
        handleRequestError();
        return;
...
export const getUpdate = () => async (dispatch, getState) => {
    ...
    const handleRequestError = () => {
        const options = {
            components: {
                a: React.createElement("a", { href: MANUAL_UPDATE_LINK, target: "_blank", rel: "noopener noreferrer" }),
            },
        };
...
export const testUpstream = ({ bootstrap_dns, upstream_dns, local_ptr_upstreams, fallback_dns }, upstream_dns_file) => async (dispatch) => {
...
export const testUpstreamWithFormValues = (formValues) => async (dispatch, getState) => {
...
export const findActiveDhcp = (selectedInterface) => async (dispatch, getState) => {
...
export const toggleBlocking = (type, domain, baseRule, baseUnblocking) => async (dispatch, getState) => {
