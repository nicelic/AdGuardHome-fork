...
const shortPollQueryLogs = async (data, filter, dispatch, currentQuery, total) => {
    const { logs, oldest } = data;
    const totalData = total || { logs };
    const previousQuery = filter?.search;
    const isQueryTheSame = typeof previousQuery === 'string' && typeof currentQuery === 'string' && previousQuery === currentQuery;
    const isShortPollingNeeded = (logs.length < QUERY_LOGS_PAGE_LIMIT || totalData.logs.length < QUERY_LOGS_PAGE_LIMIT) &&
        oldest !== '' &&
        isQueryTheSame;
...
export const getLogs = (currentQuery) => async (dispatch, getState) => {
