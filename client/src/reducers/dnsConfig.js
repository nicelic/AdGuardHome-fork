...
import { ALL_INTERFACES_IP, BLOCKING_MODES, DEFAULT_BOOTSTRAP_DNS, DEFAULT_UPSTREAM_DNS, DEFAULT_UPSTREAM_MODE, DEFAULT_UPSTREAM_TIMEOUT, DNS_REQUEST_OPTIONS, } from '../helpers/constants';
export const DEFAULT_BLOCKING_IPV4 = ALL_INTERFACES_IP;
export const DEFAULT_BLOCKING_IPV6 = '::';
const dnsConfig = handleActions({
    [actions.getDnsConfigRequest.toString()]: (state) => ({
        ...state,
        processingGetConfig: true,
    }),
    [actions.getDnsConfigFailure.toString()]: (state) => ({
        ...state,
        processingGetConfig: false,
    }),
    [actions.getDnsConfigSuccess.toString()]: (state, { payload }) => {
        const { blocking_ipv4, blocking_ipv6, upstream_dns, upstream_mode, fallback_dns, bootstrap_dns, local_ptr_upstreams, ratelimit_whitelist, ...values } = payload;
        return {
            ...state,
            ...values,
    ...
    [actions.setDnsConfigRequest.toString()]: (state) => ({
        ...state,
        processingSetConfig: true,
    }),
    [actions.setDnsConfigFailure.toString()]: (state) => ({
        ...state,
        processingSetConfig: false,
    }),
    [actions.setDnsConfigSuccess.toString()]: (state, { payload }) => ({
        ...state,
        ...payload,
        processingSetConfig: false,
    }),
