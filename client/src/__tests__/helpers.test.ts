...
describe('formatClientProtocol', () => {
    ...
    test('appends uppercase transport for doh', () => {
        expect(formatClientProtocol('doh', 'udp', translate)).toBe('DNS-over-HTTPS(UDP)');
    });

    test('appends uppercase transport for plain dns', () => {
        expect(formatClientProtocol('', 'tcp', translate)).toBe('无加密 DNS(TCP)');
    });

    test('keeps original protocol label when transport is missing', () => {
        expect(formatClientProtocol('dot', '', translate)).toBe('DNS-over-TLS');
    });
});
...
describe('redirectToCurrentProtocol', () => {
    ...
    test('does not redirect when already on standard HTTPS port', () => {
        const replace = vi.fn();
        ...
        redirectToCurrentProtocol({
            enabled: true,
            force_https: true,
            panel_server_port: 443,
            panel_server_url_path: '/myui',
            port_https: 443,
        });
