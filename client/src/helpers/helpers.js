...
import { ADDRESS_TYPES, CHECK_TIMEOUT, COMMENT_LINE_DEFAULT_TOKEN, DEFAULT_DATE_FORMAT_OPTIONS, DEFAULT_LANGUAGE, DEFAULT_TIME_FORMAT, DETAILED_DATE_FORMAT_OPTIONS, DHCP_VALUES_PLACEHOLDERS, FILTERED, FILTERED_STATUS, HTML_PAGES, R_CLIENT_ID, SCHEME_TO_PROTOCOL_MAP, STANDARD_DNS_PORT, STANDARD_HTTPS_PORT, STANDARD_WEB_PORT, SPECIAL_FILTER_ID, THEMES, } from './constants';
 ...
 */
export const formatDetailedDateTime = (dateTime) => formatDateTime(dateTime, DETAILED_DATE_FORMAT_OPTIONS);
export const getClientTransportLabel = (clientTransport) => clientTransport ? `(${clientTransport.toUpperCase()})` : '';
export const formatClientProtocol = (clientProto, clientTransport, translate = i18n.t.bind(i18n)) => {
    const protocolKey = SCHEME_TO_PROTOCOL_MAP[clientProto];
    const protocol = protocolKey ? translate(protocolKey) : '';
    return `${protocol}${getClientTransportLabel(clientTransport)}`;
};
export const normalizeLogs = (logs) => logs.map((log) => {
    const { answer, answer_dnssec, client, client_proto, client_transport, client_id, client_info, elapsedMs, question, reason, status, time, filterId, rule, rules, service_name, original_answer, upstream, cached, ecs, } = log;
...
export const normalizeTopStats = (stats) => stats.map((item) => ({
...
export const normalizeFilters = (filters) => filters
    ? filters.map((filter) => {
...
export const captitalizeWords = (text) => text
    .split(/[ -_]/g)
...
export const checkRedirect = (url, attempts = 1) => {
    ...
    const setRecursiveTimeout = (time, ...args) => setTimeout(checkRedirect, time, ...args);
    let timeout;
    fetch(url)
        .then(() => {
        rmTimeout(timeout);
...
export const normalizePanelBasePath = (panelPath = '/') => {
    const trimmedPath = panelPath.trim();
...
export const getPanelPagePath = (page, panelPath = '/') => {
    const basePath = normalizePanelBasePath(panelPath);
    if (page === HTML_PAGES.MAIN) {
        return basePath === '/' ? '/' : `${basePath}/`;
    }
    return basePath === '/' ? page : `${basePath}${page}`;
};
export const detectCurrentPanelPage = (pathname = window.location.pathname) => {
    if (pathname.endsWith(HTML_PAGES.LOGIN)) {
        return HTML_PAGES.LOGIN;
    }
    if (pathname.endsWith(HTML_PAGES.INSTALL)) {
        return HTML_PAGES.INSTALL;
    }
    return HTML_PAGES.MAIN;
};
export const getCurrentPanelBasePath = (pathname = window.location.pathname) => {
    const currentPage = detectCurrentPanelPage(pathname);
    if (currentPage === HTML_PAGES.MAIN) {
        return normalizePanelBasePath(pathname);
    }
    const basePath = pathname.slice(0, -currentPage.length) || '/';
    return normalizePanelBasePath(basePath);
};
export const redirectToCurrentProtocol = (values, httpPort = 80) => {
    const { protocol, hostname, hash, port, pathname } = window.location;
    const { enabled, force_https, panel_server_port, panel_server_url_path, port_https } = values;
    const currentPage = detectCurrentPanelPage(pathname);
    const panelHttpsPort = panel_server_port || port_https;
    const nextPath = getPanelPagePath(currentPage, panel_server_url_path || getCurrentPanelBasePath(pathname));
    const targetHash = currentPage === HTML_PAGES.MAIN ? hash : '';
    const currentHttpsPort = port ? parseInt(port, 10) : STANDARD_HTTPS_PORT;
    if (protocol !== 'https:' && enabled && force_https && panelHttpsPort) {
        const httpsPort = panelHttpsPort !== STANDARD_HTTPS_PORT ? `:${panelHttpsPort}` : '';
        checkRedirect(`https://${hostname}${httpsPort}${nextPath}${targetHash}`);
        return;
    }
    if (protocol === 'https:' && enabled && panelHttpsPort && panelHttpsPort !== currentHttpsPort) {
        const httpsPort = panelHttpsPort !== STANDARD_HTTPS_PORT ? `:${panelHttpsPort}` : '';
        checkRedirect(`https://${hostname}${httpsPort}${nextPath}${targetHash}`);
        return;
    }
    if (protocol === 'https:' && (!enabled || !panelHttpsPort)) {
        window.location.replace(`http://${hostname}:${httpPort}${nextPath}${targetHash}`);
        return;
    }
    if (pathname !== nextPath) {
        window.location.replace(`${window.location.origin}${nextPath}${targetHash}`);
    }
};
/**
 * @param {string} text
 ...
 * @returns {string}
 */
export const removeEmptyLines = (text) => splitByNewLine(text).join('\n');
/**
 * @param {string} input
 * @returns {string}
 */
export const trimLinesAndRemoveEmpty = (input) => input
    .split('\n')
    .map((line) => line.trim())
    .filter(Boolean)
    .join('\n');
 ...
 */
export const normalizeTopClients = (topClients) => topClients.reduce((acc, clientObj) => {
 ...
 * @returns {*}
 */
export const subnetMaskToBitMask = (subnetMask) => subnetMask.split('.').reduce((acc, cur) => acc - Math.log2(256 - Number(cur)), 32);
/**
 *
 * @param ipOrCidr
...
export const countClientsStatistics = (ids, autoClients) => {
    ...
    const cidrsCount = Object.entries(autoClients).reduce((acc, curr) => {
        const [id, count] = curr;
...
export const processContent = (content) => Array.isArray(content) ? content.filter(([, value]) => value).reduce((acc, val) => acc.concat(val), []) : content;
export const getObjectKeysSorted = (object, sortKey) => {
...
export const getFilterName = (filters, whitelistFilters, filterId, resolveFilterName = (filter) => (filter ? filter.name : i18n.t('unknown_filter', { filterId }))) => {
...
export const getFilterNames = (rules, filters, whitelistFilters) => rules.map(({ filter_list_id }) => getFilterName(filters, whitelistFilters, filter_list_id));
export const getRuleNames = (rules) => rules.map(({ text }) => text);
export const getFilterNameToRulesMap = (rules, filters, whitelistFilters) => rules.reduce((acc, { text, filter_list_id }) => {
...
export const getRulesToFilterList = (rules, filters, whitelistFilters, classes = {
    ...
    return (React.createElement("dl", { className: classes.list }, Object.entries(filterNameToRulesMap).reduce((acc, [filterName, rulesArr]) => acc
        .concat(rulesArr.map((rule, i) => (React.createElement("dd", { key: i, className: classes.rule }, rule))))
        .concat(React.createElement("dt", { className: classes.filter, key: classes.filter }, filterName)), [])));
};
/**
 * @param ip {string}
 ...
 * @returns {string[]}
 */
export const filterOutComments = (lines) => lines.filter((line) => !line.startsWith(COMMENT_LINE_DEFAULT_TOKEN));
/**
