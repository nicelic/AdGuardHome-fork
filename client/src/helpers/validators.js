...
import { MAX_PORT, R_CIDR, R_CIDR_IPV6, R_HOST, R_IPV4, R_IPV6, R_MAC, R_URL_REQUIRES_PROTOCOL, STANDARD_WEB_PORT, UNSAFE_PORTS, R_CLIENT_ID, R_DOMAIN, MAX_PASSWORD_LENGTH, MIN_PASSWORD_LENGTH, R_IPV4_SUBNET, R_IPV6_SUBNET, } from './constants';
...
/**
 * @param value {string}
 * @param pairedValue {string}
 * @returns {undefined|string}
 */
export const validatePairedValue = (value, pairedValue) => {
    const formattedValue = typeof value === 'string' ? value.trim() : value;
    const formattedPairedValue = typeof pairedValue === 'string' ? pairedValue.trim() : pairedValue;
    if ((formattedValue && formattedPairedValue) || (!formattedValue && !formattedPairedValue)) {
        return undefined;
    }
    return i18next.t('form_error_required');
};
 ...
 * @returns {undefined|string}
 */
export const validateURLPathPrefix = (value) => {
    if (!value) {
        return undefined;
    }
    const formattedValue = value.trim();
    ...
    const normalizedValue = formattedValue.startsWith('/') ? formattedValue.slice(1) : formattedValue;
    const isInvalidPath = normalizedValue
        .split('/')
 ...
 * @returns {undefined|string}
 */
export const validateAbsolutePath = (value) => {
...
export const validatePlainDns = (value, allValues) => {
    ...
    const hasEncryptedDNSProtocol = !!allValues.port_https ||
        !!allValues.port_dns_over_tls ||
        !!allValues.port_dns_over_quic ||
        !!allValues.port_dnscrypt;
    if (!value && (!enabled || !hasEncryptedDNSProtocol)) {
