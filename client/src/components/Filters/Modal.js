...
class Modal extends Component {
    ...
    render() {
        const { isOpen, processingAddFilter, processingConfigFilter, handleSubmit, modalType, currentFilterData, whitelist, toggleFilteringModal, filters, t, filtersCatalog, } = this.props;
        let initialValues;
        let selectedSources;
        switch (modalType) {
        ...
        return (React.createElement(ReactModal, { className: "Modal__Bootstrap modal-dialog modal-dialog-centered", closeTimeoutMS: 0, isOpen: isOpen, onRequestClose: this.closeModal },
            React.createElement("div", { className: "modal-content" },
                React.createElement("div", { className: "modal-header" },
                    title && React.createElement("h4", { className: "modal-title" }, title),
                    React.createElement("button", { type: "button", className: "close", onClick: this.closeModal },
                        React.createElement("span", { className: "sr-only" }, "Close"))),
                React.createElement(Form, { selectedSources: selectedSources, initialValues: initialValues, modalType: modalType, onSubmit: handleSubmit, processingAddFilter: processingAddFilter, processingConfigFilter: processingConfigFilter, closeModal: this.closeModal, whitelist: whitelist, toggleFilteringModal: toggleFilteringModal }))));
    }
}
