...
const processContent = (data, _buttonType) => Object.entries(data).map(([key, value]) => {
    if (!value) {
        return null;
    }
    ...
    return isHidden ? null : (React.createElement("div", { className: "grid__row", key: key },
        React.createElement("div", { className: classNames(`key__${key}`, keyClass, {
                'font-weight-bold': isBoolean && value === true,
            }) },
            React.createElement(Trans, null, isButton ? value : key)),
        React.createElement("div", { className: `value__${key} text-pre text-truncate` },
            React.createElement(Trans, null, isTitle || isButton || isBoolean ? '' : value || '—'))));
});
const Logs = () => {
    ...
    const { enabled, processingGetConfig, processingGetLogs, anonymize_client_ip: anonymizeClientIp, } = useSelector((state) => state.queryLogs, shallowEqual);
    ...
    const renderPage = () => (React.createElement(React.Fragment, null,
        React.createElement(FormProvider, { ...formMethods },
            React.createElement(Filters, { setIsLoading: setIsLoading, processingGetLogs: processingGetLogs })),
        React.createElement(InfiniteTable, { isLoading: isLoading, items: logs, isSmallScreen: isSmallScreen, setDetailedDataCurrent: setDetailedDataCurrent, setButtonType: setButtonType, setModalOpened: setModalOpened, currentQuery: currentQuery }),
        React.createElement(Modal, { portalClassName: "grid", isOpen: isSmallScreen && isModalOpened, onRequestClose: closeModal, style: {
                content: {
                    width: 'calc(100% - 32px)',
                    height: 'fit-content',
                    left: '50%',
            ...
            React.createElement("div", { className: "logs__modal-wrap" },
                React.createElement("svg", { className: "icon icon--24 icon-cross d-block cursor--pointer", onClick: closeModal },
                    React.createElement("use", { xlinkHref: "#cross" })),
                processContent(detailedDataCurrent, buttonType)))));
    return (React.createElement(React.Fragment, null,
        enabled && (React.createElement(React.Fragment, null,
            processingGetConfig && React.createElement(Loading, null),
            anonymizeClientIp && React.createElement(AnonymizerNotification, null),
            !processingGetConfig && renderPage())),
        !enabled && !processingGetConfig && React.createElement(Disabled, null)));
};
export default Logs;
