...
const ClientCell = ({ client, client_id, client_info, domain, reason }) => {
    ...
    const renderBlockingButton = (isFiltered, domain) => {
        ...
        const getOptions = (options) => {
            ...
            return (React.createElement(React.Fragment, null, options.map(({ name, onClick, disabled, className }) => (React.createElement("button", { key: name, className: classNames('button-action--arrow-option px-4 py-1', className), onClick: onClick, disabled: disabled }, t(name))))));
        };
        ...
        return (React.createElement("div", { className: containerClass },
            ...
            isOptionsOpened && (React.createElement(IconTooltip, { className: "icon24", tooltipClass: "button-action--arrow-option-container", xlinkHref: "bullets", triggerClass: "btn btn-icon btn-sm px-0 button-action__hidden-trigger", content: content, placement: "bottom-end", trigger: "click", onVisibilityChange: setOptionsOpened, defaultTooltipShown: true, delayHide: 0 }))));
    };
    return (React.createElement("div", { className: "o-hidden h-100 logs__cell logs__cell--client", role: "gridcell" },
        React.createElement(IconTooltip, { className: hintClass, columnClass: "grid grid--limited", tooltipClass: "px-5 pb-5 pt-4", xlinkHref: "question", contentItemClass: "text-truncate key-colon o-hidden", title: "client_details", content: processedData, placement: "bottom" }),
        React.createElement("div", { className: nameClass },
            React.createElement("div", { "data-tip": true, "data-for": id }, renderFormattedClientCell(client, clientInfo, isDetailed, true)),
            isDetailed && clientName && !whoisAvailable && (React.createElement(Link, { className: "detailed-info d-none d-sm-block logs__text logs__text--link logs__text--client", to: `logs?search="${encodeURIComponent(clientName)}"`, title: clientName }, clientName))),
