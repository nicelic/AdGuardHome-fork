...
const CacheConfig = () => {
    ...
    const { cache_enabled, cache_size, cache_ttl_max, cache_ttl_min, cache_optimistic } = useSelector((state) => state.dnsConfig, shallowEqual);
    const handleFormSubmit = (values) => {
        const completedFields = replaceEmptyStringsWithZeroes(values);
        dispatch(setDnsConfig(completedFields));
    };
    return (React.createElement(Card, { title: t('dns_cache_config'), subtitle: t('dns_cache_config_desc'), bodyType: "card-body box-body--settings", id: "dns-config" },
