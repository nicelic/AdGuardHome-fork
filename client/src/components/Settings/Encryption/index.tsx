...
import { EncryptionData, EncryptionPathPair } from '../../../initialState';

type TLSBlockSubmitConfig = {
    certificateSource: string;
    keySource: string;
    privateKeySaved: boolean;
    certificateKeyPairs: EncryptionPathPair[];
    prefix?: 'panel_';
};

type Props = {
    encryption: EncryptionData;
    setTlsConfig: (values: Partial<EncryptionData>) => Promise<unknown>;
...
export const Encryption = ({ encryption, setTlsConfig, validateTlsConfig }: Props) => {
    ...
    const getPathPairs = useCallback((pairs: EncryptionPathPair[], certificatePath = '', privateKeyPath = '') => {
        if (pairs.length > 0) {
            return pairs;
        }
    ...
    const normalizeTLSBlockSubmitValues = useCallback(
        (config: Record<string, any>, block: TLSBlockSubmitConfig) => {
            ...
            const normalizedPathPairs = certificateKeyPairs.filter(
                (pair) => pair.certificate_path.trim() || pair.private_key_path.trim(),
            );
    ...
    const initialValues = useMemo((): EncryptionFormValues => {
        ...
        const panelPathPairs = getPathPairs(
            panel_certificate_key_pairs,
            panel_certificate_path,
            panel_private_key_path,
        );
    ...
    const handleFormSubmit = useCallback(
        async (values: any) => {
            ...
            const dnsCertificateRequired = Boolean(
                submitValues.enabled
            ...
            const dnsCanApply = !dnsCertificateRequired || (
                validation.can_apply
                && validation.valid_key
                && validation.valid_cert
                && validation.valid_pair
            );
            const panelCanApply = !panelCertificateRequired || (
                validation.panel_can_apply
    ...
    const debouncedConfigValidation = useMemo<DebouncedFunc<(values: EncryptionFormValues) => void>>(
