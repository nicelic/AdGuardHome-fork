...
const App = () => {
    ...
    const { language, isCoreRunning, isUpdateAvailable, processing, theme } = useSelector((state) => state.dashboard, shallowEqual);
    ...
    return (React.createElement(HashRouter, { hashType: "noslash" },
        updateAvailable && (React.createElement(React.Fragment, null,
            React.createElement(UpdateTopline, null),
            React.createElement(UpdateOverlay, null))),
        !processingEncryption && React.createElement(EncryptionTopline, null),
        React.createElement(LoadingBar, { className: "loading-bar", updateTime: 1000 }),
        React.createElement(Header, null),
        React.createElement(ProtectionTimer, null),
        React.createElement("div", { className: "container container--wrap pb-5 pt-5" },
            processing && React.createElement(Loading, null),
            !isCoreRunning && (React.createElement("div", { className: "row row-cards" },
                React.createElement("div", { className: "col-lg-12" },
                    React.createElement(Status, { reloadPage: reloadPage, message: "dns_start" }),
                    React.createElement(Loading, null)))),
            !processing &&
                isCoreRunning &&
                ROUTES.map((route, index) => (React.createElement(Route, { key: index, exact: route.exact, path: route.path, component: route.component })))),
