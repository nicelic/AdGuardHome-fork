...
* DNS access settings
	...
	* Set access settings
* Rewrites
	* API: Get classic rewrite text
	* API: Update classic rewrite text
* Services Filter
...
Request:
	...
	"certificate_key_pairs":[
		{"certificate_path":"...","private_key_path":"..."}
	] // if set, certificate_chain and private_key must be empty
	}
...
`blocking_mode`:
* default: Same as Null IP for Adblock-style rule;  respond with the IP address specified in the rule when blocked by /etc/hosts-style rule
...
`disable_ipv4`: Drop all DNS queries for IPv4 addresses (type A) and remove IPv4 hints from HTTPS responses.

`disable_ipv6`: Drop all DNS queries for IPv6 addresses (type AAAA) and remove IPv6 hints from HTTPS responses.
...
`rules` is a multiline text block.  Each enabled line must be in the form:

	[left matchers]:[ipv4 values][ipv6 values][cname values]

Optional ASCII whitespace is accepted around `:` and between groups.  The text
is stored as sent, without line reformatting.

Rules are matched from top to bottom.  The first enabled line whose left group
contains a matching value wins.  If any matcher inside that left group matches,
all values from the three right groups are returned together.  Lines beginning
with `#` are treated as disabled comments.
...
Request:
	...
	PUT /control/rewrite/text

	{
		rules: "[example.org *.example.org]:[1.1.1.1][2001:db8::1][alias.example]"
	}
...
When a new DNS request is received and processed, we store information about this event in "query log".  It is a file on disk in JSON format:
	...
	"QT":"...", // question type
	"QC":"...", // question class
	"CP":"" | "doh" | "dot" | "doq" | "dnscrypt", // client connection protocol
	"CT":"" | "tcp" | "udp", // client connection transport
	"Answer":"base64 data",
...
Response:
	...
	"data":[
	{
		...
		"answer_dnssec": true,
		"client":"127.0.0.1",
		"client_proto": "" (plain) | "doh" | "dot" | "doq" | "dnscrypt",
		"client_transport": "" | "tcp" | "udp",
		"elapsedMs":"0.098403",
		"filterId":1,
		"question":{
			"class":"IN",
