...
// ClientTransport values are names of the client transport protocols.
...
// Client transport names.
const (
	ClientTransportTCP     ClientTransport = "tcp"
	ClientTransportUDP     ClientTransport = "udp"
	ClientTransportUnknown ClientTransport = ""
)
...
// NewClientTransport validates that the client transport name is valid and
// returns the name as a [ClientTransport].
func NewClientTransport(s string) (ct ClientTransport, err error) {
	switch ct = ClientTransport(s); ct {
	case
		ClientTransportTCP,
		ClientTransportUDP,
		ClientTransportUnknown:

		return ct, nil
	default:
		return "", fmt.Errorf("invalid client transport: %q", s)
	}
}
...
func newLogEntry(ctx context.Context, logger *slog.Logger, params *AddParams) (entry *logEntry) {
	...
	entry = &logEntry{
		...
		ClientTransport: params.ClientTransport,
