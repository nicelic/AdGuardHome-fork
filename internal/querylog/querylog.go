...
type AddParams struct {
	...
	// ClientTransport is the underlying transport used by the client.
...
func (p *AddParams) validate() (err error) {
	...
	_, err = NewClientProto(string(p.ClientProto))
	if err != nil {
		return err
	}

	_, err = NewClientTransport(string(p.ClientTransport))
	if err != nil {
		return err
	}
