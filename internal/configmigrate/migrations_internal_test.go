...
func TestUpgradeSchema34to35(t *testing.T) {
	t.Parallel()

	const newSchemaVer = 35
	...
	testCases := []struct {
		...
		in: yobj{
			"filtering": yobj{
				"rewrites": "[example.org]:[192.0.2.1][][]\n",
			},
		},
		want: yobj{
			"filtering": yobj{
		...
		in: yobj{
			"filtering": yobj{
				"rewrites": yarr{
					...
					yobj{
						"domain":  "nil-enabled.example",
						"answer":  "nil-target.example",
						"enabled": nil,
					},
				},
			},
		...
		want: yobj{
			"filtering": yobj{
				"rewrites": "[example.org]:[192.0.2.1][2001:db8::1][alias.example]\n" +
					"[nil-enabled.example]:[][][nil-target.example]\n" +
					"# [disabled.example]:[192.0.2.2][][]\n" +
					"# [keep.example]:[][][] legacy rewrite answer \"A\" cannot be represented by text rewrites\n",
			},
			"schema_version": newSchemaVer,
		},
	}}
