...
func (s *Server) filterDNSRequest(
	...
	case
		...
		if res.DNSRewriteResult != nil {
			if err = s.filterClassicDNSRewrite(ctx, req, res, pctx); err != nil {
				return nil, err
			}
...
func isRewrittenCNAME(res *filtering.Result) (ok bool) {
	...
	case
		...
		filtering.Rewritten,
		filtering.RewrittenRule:
		if res.Reason == filtering.Rewritten && res.DNSRewriteResult != nil {
			return false
		}
...
// removeIPv4Hints deletes IPv4 hints from RR values.
func removeIPv4Hints(rr *dns.HTTPS) {
	rr.Value = slices.DeleteFunc(rr.Value, func(kv dns.SVCBKeyValue) (del bool) {
		_, ok := kv.(*dns.SVCBIPv4Hint)
...
// filters of the server filters.  It removes IPv4 and IPv6 hints if resolving
// them is disabled.  All arguments must not be nil.
