...
func (s *Server) filterDNSRewriteResponse(
	...
	case dns.TypeCNAME:
		return s.ansFromDNSRewriteCNAME(v, req)
...
// ansFromDNSRewriteCNAME creates a new answer resource record from the CNAME
// dnsrewrite rule data.
func (s *Server) ansFromDNSRewriteCNAME(v rules.RRValue, req *dns.Msg) (ans dns.RR, err error) {
	cname, ok := v.(string)
	if !ok {
		return nil, fmt.Errorf("value for rr type %s has type %T, not string", dns.Type(dns.TypeCNAME), v)
	}

	return s.genAnswerCNAME(req, cname), nil
}
...
// filterClassicDNSRewrite handles classic text DNS rewrites.  It always
// returns all configured CNAME, A, and AAAA values in that order.
func (s *Server) filterClassicDNSRewrite(
	ctx context.Context,
	req *dns.Msg,
	res *filtering.Result,
	pctx *proxy.DNSContext,
	...
	dnsrr := res.DNSRewriteResult
	if dnsrr == nil {
		return errors.Error("no classic dns rewrite rule content")
	}
	...
	if dnsrr.Response == nil {
		return errors.Error("no classic dns rewrite rule responses")
	}

	for _, rrType := range []rules.RRType{dns.TypeCNAME, dns.TypeA, dns.TypeAAAA} {
		values := dnsrr.Response[rrType]
		for i, v := range values {
			var ans dns.RR
			ans, err = s.filterDNSRewriteResponse(ctx, req, rrType, v)
			if err != nil {
				return fmt.Errorf("classic dns rewrite response for %s[%d]: %w", dns.Type(rrType), i, err)
			}
