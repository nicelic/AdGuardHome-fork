...
func (s *Server) processInitial(
	...
	qt := q.Qtype
	if (s.conf.ADisabled && qt == dns.TypeA) || (s.conf.AAAADisabled && qt == dns.TypeAAAA) {
...
func (s *Server) makeDDRResponse(req *dns.Msg) (resp *dns.Msg) {
	...
	if req.Question[0].Qtype != dns.TypeSVCB || aghtls.IsWildcardServerName(s.conf.TLSConf.ServerName) {
		return resp
	}
	...
	dohPath := s.conf.TLSConf.HTTPSPath
	if dohPath == "" {
		dohPath = "/dns-query"
	}
	...
	for _, addr := range s.conf.TLSConf.HTTPSListenAddrs {
		values := []dns.SVCBKeyValue{
			...
			&dns.SVCBDoHPath{Template: dohPath + "{?dns}"},
		}
