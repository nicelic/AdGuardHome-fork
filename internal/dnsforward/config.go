...
import (
	...
	"github.com/nicelic/AdGuardHome-fork/internal/aghslog"
...
type Config struct {
	...
	// ADisabled, if true, respond with an empty answer to all A requests.
	ADisabled bool `yaml:"a_disabled"`
...
type TLSConfig struct {
	...
	// Certificates are the TLS certificates used for TLS connections.  When it
	// is empty and [TLSConfig.Cert] is set, the single certificate is used.
	...
	// HTTPSRoutes are the HTTP route patterns used for DoH requests.
	HTTPSRoutes []string

	// HTTPSPath is the DoH request path announced via DDR.
	HTTPSPath string

	// ServeHTTPS controls whether the dedicated DoH HTTPS listener should be
	// started by dnsproxy.
	ServeHTTPS bool
...
// certificates returns all configured certificates.
func (conf *TLSConfig) certificates() (certs []tls.Certificate) {
	if len(conf.Certificates) > 0 {
		return conf.Certificates
	}
	...
	return []tls.Certificate{*conf.Cert}
}
...
const (
	...
	// DefaultUpstreamMode is the default strategy used for upstream DNS
	// selection.
	DefaultUpstreamMode = UpstreamModeParallel
)
...
func (s *Server) initDefaultSettings() {
	...
	if s.conf.UpstreamMode == "" {
		s.conf.UpstreamMode = DefaultUpstreamMode
	}
...
func (s *Server) prepareTLS(ctx context.Context, proxyConf *proxy.Config) (err error) {
	...
	certs := s.conf.TLSConf.certificates()
	if len(certs) == 0 {
		return nil
	}
	...
	primaryCert := aghtls.FindTLSCertificate(certs, s.conf.TLSConf.ServerName)
	if primaryCert == nil {
		primaryCert = &certs[0]
	}

	leaf, err := aghtls.CertificateLeaf(primaryCert)
	if err != nil {
		return fmt.Errorf("getting primary tls certificate leaf: %w", err)
	}

	s.hasIPAddrs = aghtls.CertificateHasIP(leaf)

	if s.conf.TLSConf.StrictSNICheck {
		s.dnsNames, err = aghtls.CertificateNames(certs)
		if err != nil {
			return fmt.Errorf("collecting tls certificate names: %w", err)
		}

		s.logger.DebugContext(ctx, "using certificate names for strict sni check", "dns_names", s.dnsNames)
	}

	if s.conf.TLSConf.ServeHTTPS && len(s.conf.TLSConf.HTTPSListenAddrs) > 0 {
		proxyConf.HTTPConfig.ListenAddresses = slices.Clone(s.conf.TLSConf.HTTPSListenAddrs)
		proxyConf.HTTPConfig.Routes = slices.Clone(s.conf.TLSConf.HTTPSRoutes)
		proxyConf.HTTPConfig.HTTP3Enabled = s.conf.ServeHTTP3
	}
...
func anyNameMatches(dnsNames []string, sni string) (ok bool) {
	return aghtls.MatchCertificateName(dnsNames, sni)
}
...
func (s *Server) onGetCertificate(ch *tls.ClientHelloInfo) (*tls.Certificate, error) {
	...
	if ch.Conn != nil {
		localIP = netutil.NetAddrToAddrPort(ch.Conn.LocalAddr()).Addr()
	}
	cert, err := aghtls.SelectCertificate(certs, aghtls.SelectionOptions{
		RequestedServerName:    ch.ServerName,
		ServiceName:            s.conf.TLSConf.ServerName,
		LocalIP:                localIP,
		Strict:                 s.conf.TLSConf.StrictSNICheck,
