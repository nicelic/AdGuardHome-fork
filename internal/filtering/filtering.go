...
type Config struct {
	...
	// Rewrites is the raw multiline classic DNS rewrites text.
	Rewrites string `yaml:"rewrites"`

	// rewrites is the compiled form of [Rewrites].
	rewrites []*textRewriteRule `yaml:"-"`

	// SimpleAllowlist is the raw multiline simple allowlist text.
	SimpleAllowlist string `yaml:"simple_allowlist"`

	// simpleAllowRules is the compiled form of [SimpleAllowlist].
	simpleAllowRules []*simpleListRule `yaml:"-"`

	// SimpleBlocklist is the raw multiline simple blocklist text.
	SimpleBlocklist string `yaml:"simple_blocklist"`

	// simpleBlockRules is the compiled form of [SimpleBlocklist].
	simpleBlockRules []*simpleListRule `yaml:"-"`
	...
	// RewritesEnabled indicates whether classic text rewrites are applied.
...
type DNSFilter struct {
	...
	rulesStorageBlock    *filterlist.RuleStorage
...
func (d *DNSFilter) setFilters(
	...
	return d.initFiltering(ctx, userFilter, allowFilters, blockFilters)
}
...
func (d *DNSFilter) reset(ctx context.Context) {
	if d.rulesStorageUser != nil {
		if err := d.rulesStorageUser.Close(); err != nil {
			d.logger.ErrorContext(ctx, "closing user rules storage", slogutil.KeyError, err)
		}
	}
	...
	if d.rulesStorageBlock != nil {
		if err := d.rulesStorageBlock.Close(); err != nil {
			d.logger.ErrorContext(ctx, "closing block rules storage", slogutil.KeyError, err)
		}
	}
}
...
func (d *DNSFilter) initFiltering(
	ctx context.Context,
	userFilter Filter,
	allowFilters []Filter,
	blockFilters []Filter,
) (err error) {
	rulesStorageUser, err := newRuleStorage([]Filter{userFilter})
	if err != nil {
		return err
	}

	rulesStorageBlock, err := newRuleStorage(blockFilters)
	if err != nil {
		return err
	}
	...
	filteringEngineUser := urlfilter.NewDNSEngine(rulesStorageUser)
	filteringEngineAllow := urlfilter.NewDNSEngine(rulesStorageAllow)
	filteringEngineBlock := urlfilter.NewDNSEngine(rulesStorageBlock)
	...
	func() {
		...
		d.rulesStorageBlock = rulesStorageBlock
		d.filteringEngineBlock = filteringEngineBlock
	}()
...
func (d *DNSFilter) matchHost(
	...
	if setts.ProtectionEnabled {
		res = d.matchSimpleAllowlist(host)
		if res.Reason.Matched() {
			return res, nil
		}

		res = d.matchSimpleBlocklist(host)
		if res.Reason.Matched() {
			return res, nil
		}
	}
	...
	res, err = d.matchFilterEngine(host, rrtype, ufReq, d.filteringEngineUser, setts)
	if err != nil {
		return Result{}, err
	} else if res.Reason.Matched() {
		return res, nil
	}
	...
	return d.matchFilterEngine(host, rrtype, ufReq, d.filteringEngineBlock, setts)
}

// matchFilterEngine checks the host against a single filtering engine.
func (d *DNSFilter) matchFilterEngine(
	host string,
	rrtype uint16,
	ufReq *urlfilter.DNSRequest,
	...
	dnsres, matchedEngine := engine.MatchRequest(ufReq)
...
func New(c *Config, blockFilters []Filter) (d *DNSFilter, err error) {
	...
	err = d.prepareSimpleLists()
	if err != nil {
		return nil, fmt.Errorf("simple lists: preparing: %w", err)
	}
	...
	if blockFilters != nil {
		err = d.initFiltering(ctx, Filter{}, nil, blockFilters)
		if err != nil {
			d.Close()
...
func (d *DNSFilter) updatesLoop(ctx context.Context) {
	...
	for {
		...
		case params := <-d.filtersInitializerChan:
			err := d.initFiltering(ctx, params.userFilter, params.allowFilters, params.blockFilters)
			if err != nil {
