...
type Result struct {
	// DNSRewriteResult is the DNS rewrite result payload.
	DNSRewriteResult *DNSRewriteResult `json:",omitempty"`

	// CanonName is the first CNAME value from the rewrite result.  It is empty
	...
	// IPList contains all IP values from the rewrite result.  It is empty unless
	// Reason is set to Rewritten.
