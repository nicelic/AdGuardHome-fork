...
type checkHostResp struct {
	...
	// for Rewrite:
	CanonName string       `json:"cname"`            // first CNAME value
	CNames    []string     `json:"cnames,omitempty"` // list of CNAME values
...
// resultCNAMEs returns all CNAME values from result's DNS rewrite payload.
func resultCNAMEs(result *Result) (cnames []string) {
	if result == nil || result.DNSRewriteResult == nil || result.DNSRewriteResult.Response == nil {
		if result != nil && result.CanonName != "" {
			return []string{result.CanonName}
		}

		return nil
	}

	for _, value := range result.DNSRewriteResult.Response[dns.TypeCNAME] {
		cname, ok := value.(string)
		if !ok {
			continue
		}
	...
	if len(cnames) == 0 && result.CanonName != "" {
		cnames = append(cnames, result.CanonName)
	}
...
func (d *DNSFilter) RegisterFilteringHandlers() {
	...
	registerHTTP(http.MethodGet, "/control/rewrite/text", d.handleRewriteTextGet)
	registerHTTP(http.MethodGet, "/control/rewrite/settings", d.handleRewriteSettings)
	registerHTTP(http.MethodPut, "/control/rewrite/text", d.handleRewriteTextUpdate)
	registerHTTP(http.MethodPut, "/control/rewrite/settings/update", d.handleRewriteSettingsUpdate)
	registerHTTP(http.MethodGet, "/control/simple_allowlist/text", d.handleSimpleAllowlistTextGet)
	registerHTTP(http.MethodPut, "/control/simple_allowlist/text", d.handleSimpleAllowlistTextUpdate)
	registerHTTP(http.MethodGet, "/control/simple_blocklist/text", d.handleSimpleBlocklistTextGet)
	registerHTTP(http.MethodPut, "/control/simple_blocklist/text", d.handleSimpleBlocklistTextUpdate)
