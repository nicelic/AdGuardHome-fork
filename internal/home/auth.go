...
// updateUserCredentials updates the login and password for the provided user
// and invalidates all current sessions.
func (a *auth) updateUserCredentials(
	ctx context.Context,
	cur *aghuser.User,
	newName string,
	newPassword string,
	...
	login, err := aghuser.NewLogin(newName)
	if err != nil {
	...
	err = a.users.Update(ctx, &aghuser.User{
		ID:       cur.ID,
		Login:    login,
		Password: aghuser.NewDefaultPassword(string(hash)),
	})
	if err != nil {
		return fmt.Errorf("updating user: %w", err)
	}

	err = a.sessions.DeleteAll(ctx)
	if err != nil {
		return fmt.Errorf("deleting sessions: %w", err)
	}

	a.logger.InfoContext(ctx, "updated user credentials", "old_login", cur.Login, "new_login", login)
