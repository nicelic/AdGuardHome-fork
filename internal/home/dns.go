...
func initDNS(
	...
	registerDoHHandlers(dnsOverHTTPSRoutes(tlsMgr.extendedTLSConfig()))
...
func newDNSTLSConfig(
	...
	dnsConf = &dnsforward.TLSConfig{
		...
		HTTPSPath:      effectiveDNSOverHTTPSURLPath(extTLSConf),
		HTTPSRoutes:    dnsOverHTTPSRoutes(extTLSConf),
		ServeHTTPS:     shouldServeDoHOnDNSProxy(extTLSConf),
	}
	...
	if len(extTLSConf.Certificates) == 0 {
		err = errors.Error("parsing tls key pair: empty certificate list")
	...
	dnsConf.Cert = &extTLSConf.Certificates[0]
	dnsConf.Certificates = slices.Clone(extTLSConf.Certificates)
...
func getDNSEncryption(tlsMgr *tlsManager) (de dnsEncryption) {
	...
	hostname := advertisedDNSServerName(extTLSConf)
	if !extTLSConf.Enabled || hostname == "" {
	...
	if extTLSConf.PortHTTPS != 0 {
		...
		de.https = (&url.URL{
			...
			Host:   addr,
			Path:   effectiveDNSOverHTTPSURLPath(extTLSConf),
		}).String()
	}
...
// registerDoHHandlers registers DoH bridge handlers on the given routes.
func registerDoHHandlers(routes []string) {
	registerDoHBridgeHandlers(routes)
}
