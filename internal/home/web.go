...
type webAPI struct {
	...
	// certManager provides certificate center APIs and storage.
...
func newWebAPI(ctx context.Context, conf *webAPIConfig) (w *webAPI) {
	...
	w = &webAPI{
		...
		auth:         conf.auth,
		certManager:  newCertificateManagerService(conf.logger.With("module", "certificate_manager"), conf.workDir, ctx),
		startTime:    time.Now(),
	}
...
func (web *webAPI) tlsConfigChanged(ctx context.Context, tlsConf *tlsConfigSettings) {
	...
	enabled := tlsConf.Enabled &&
		effectivePanelServerPort(tlsConf) != 0 &&
		len(tlsConf.PanelCertificates) != 0
	var certs []tls.Certificate
	var cert tls.Certificate
	if enabled {
		certs = slices.Clone(tlsConf.PanelCertificates)
		cert = certs[0]
	}
...
func (web *webAPI) start(ctx context.Context) {
	...
	for !web.httpsServer.inShutdown {
		...
		hdlr = web.wrapPanelHandler(hdlr)
...
func (web *webAPI) serveTLS(ctx context.Context) (next bool) {
	...
	portHTTPS = effectivePanelServerPort(extTLSConf)
	...
	getCertificate := func(ch *tls.ClientHelloInfo) (*tls.Certificate, error) {
		...
		cert, err := selectWebCertificate(web.httpsServer.certs, extTLSConf, ch.ServerName, localIP)
		if err == nil {
			return cert, nil
		}
	...
	web.httpsServer.server = &http.Server{
		Addr:    addr,
		Handler: web.wrapPanelHandler(web.auth.middleware().Wrap(hdlr)),
		TLSConfig: &tls.Config{
			Certificates:   web.httpsServer.certs,
...
func (web *webAPI) mustStartHTTP3(ctx context.Context, address string) {
	...
	getCertificate := func(ch *tls.ClientHelloInfo) (*tls.Certificate, error) {
		...
		web.logger.WarnContext(
			context.TODO(),
			"rejecting http3 client hello",
			"server_name", ch.ServerName,
			"local_ip", localIP,
			slogutil.KeyError, err,
		)
	...
	web.httpsServer.server3 = &http3.Server{
		...
		Handler: web.wrapPanelHandler(web.auth.middleware().Wrap(withMiddlewares(web.conf.mux, limitRequestBody))),
	}
