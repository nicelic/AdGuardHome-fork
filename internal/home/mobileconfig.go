...
func encodeMobileConfig(d *dnsSettings, clientID string, dohPath string) ([]byte, error) {
	var dspName string
	...
	case dnsProtoHTTPS:
		...
		u := &url.URL{
			...
			Host:   d.ServerName,
			Path:   path.Join(dohPath, clientID),
		}
...
func (m *mobileConfigHandler) handleMobileConfig(w http.ResponseWriter, r *http.Request, dnsp string) {
	...
	mobileconfig, err := encodeMobileConfig(d, clientID, effectiveDNSOverHTTPSURLPath(&config.TLS))
	if err != nil {
