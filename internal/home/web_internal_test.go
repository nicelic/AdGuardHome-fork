...
func TestWebAPI_wrapPanelHandler(t *testing.T) {
	storeGlobals(t)

	prevDelay := panelPathInvalidDelay
	panelPathInvalidDelay = 0
	t.Cleanup(func() {
		panelPathInvalidDelay = prevDelay
	})
	...
	var got requestState
	wrapped := web.wrapPanelHandler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		got = requestState{
			path:   r.URL.Path,
			orig:   originalPanelRequestPath(r),
			prefix: panelRequestPrefix(r),
		}
		w.WriteHeader(http.StatusNoContent)
	}))

	t.Run("trim_panel_prefix", func(t *testing.T) {
		got = requestState{}

		r := httptest.NewRequest(http.MethodGet, "http://127.0.0.1/myui/login.html?next=%2F", nil)
		w := httptest.NewRecorder()
		wrapped.ServeHTTP(w, r)

		assert.Equal(t, http.StatusNoContent, w.Code)
		assert.Equal(t, requestState{
			path:   "/login.html",
			orig:   "/myui/login.html",
			prefix: "/myui",
		}, got)
	})

	t.Run("trim_panel_root", func(t *testing.T) {
		got = requestState{}

		r := httptest.NewRequest(http.MethodGet, "http://127.0.0.1/myui/", nil)
		w := httptest.NewRecorder()
		wrapped.ServeHTTP(w, r)
	...
	t.Run("redirect_bare_panel_prefix", func(t *testing.T) {
		got = requestState{}

		r := httptest.NewRequest(http.MethodGet, "http://127.0.0.1/myui?next=%2F", nil)
		w := httptest.NewRecorder()
		wrapped.ServeHTTP(w, r)

		assert.Equal(t, http.StatusPermanentRedirect, w.Code)
		assert.Equal(t, "http://127.0.0.1/myui/?next=%2F", w.Header().Get("Location"))
		assert.Equal(t, requestState{}, got)
	})

	t.Run("shared_doh_stays_outside_panel_prefix", func(t *testing.T) {
		got = requestState{}

		r := httptest.NewRequest(http.MethodGet, "http://127.0.0.1/secure-dns/cli42", nil)
		w := httptest.NewRecorder()
		wrapped.ServeHTTP(w, r)

		assert.Equal(t, http.StatusNoContent, w.Code)
		assert.Equal(t, requestState{
			path:   "/secure-dns/cli42",
			orig:   "/secure-dns/cli42",
			prefix: "/",
		}, got)
	})

	t.Run("shared_doh_under_panel_prefix_is_rejected", func(t *testing.T) {
		got = requestState{}

		r := httptest.NewRequest(http.MethodGet, "http://127.0.0.1/myui/secure-dns/cli42", nil)
		w := httptest.NewRecorder()
		wrapped.ServeHTTP(w, r)

		assert.Equal(t, http.StatusNotFound, w.Code)
		assert.Equal(t, "text/html", w.Header().Get("Content-Type"))
		assert.Equal(t, nginx404Page, w.Body.String())
		assert.Equal(t, requestState{}, got)
	})

	t.Run("separated_panel_rejects_doh_on_web", func(t *testing.T) {
		got = requestState{}
		config.TLS.PanelServerPort = 3000

		r := httptest.NewRequest(http.MethodGet, "http://127.0.0.1/secure-dns", nil)
		w := httptest.NewRecorder()
		wrapped.ServeHTTP(w, r)
	...
	t.Run("invalid_path_returns_nginx_404", func(t *testing.T) {
		got = requestState{}
		config.TLS.PanelServerPort = 0

		r := httptest.NewRequest(http.MethodGet, "http://127.0.0.1/not-myui", nil)
		w := httptest.NewRecorder()
		wrapped.ServeHTTP(w, r)
