...
type testSessionStorage struct {
	...
	onDeleteAll     func(ctx context.Context) (err error)
...
func newTestSessionStorage() (ts *testSessionStorage) {
	return &testSessionStorage{
		...
		onDeleteAll: func(ctx context.Context) (_ error) {
...
// DeleteAll implements the [aghuser.SessionStorage] interface for
// *testSessionStorage.
func (ts *testSessionStorage) DeleteAll(ctx context.Context) (err error) {
	return ts.onDeleteAll(ctx)
}
...
type testUsersDB struct {
	...
	onUpdate  func(ctx context.Context, u *aghuser.User) (err error)
}
...
func newTestUsersDB() (ts *testUsersDB) {
	return &testUsersDB{
		...
		onUpdate: func(ctx context.Context, u *aghuser.User) (_ error) {
			panic(testutil.UnexpectedCall(ctx, u))
		},
	}
}
...
// Update implements the [aghuser.DB] interface for *testUsersDB.
func (db *testUsersDB) Update(ctx context.Context, u *aghuser.User) (err error) {
	return db.onUpdate(ctx, u)
}
...
func TestAuthMiddlewareDefault_public_tlsDoHPath(t *testing.T) {
	storeGlobals(t)
	...
	config = &configuration{
		TLS: tlsConfigSettings{
			...
			DNSOverHTTPSURLPath: "/secure-dns",
		},
	}

	user := newTestUser(t, testPassword, aghuser.Login(testUsername))
	usersDB := newTestUsersDB()
	usersDB.onAll = func(_ context.Context) (us []*aghuser.User, err error) {
		return []*aghuser.User{user}, nil
	}
	...
	testCases := []struct {
		...
		req:      httptest.NewRequest(http.MethodGet, "/secure-dns", nil),
		name:     "custom_doh",
		wantCode: http.StatusOK,
	}, {
		req:      httptest.NewRequest(http.MethodGet, "/secure-dns/client-1", nil),
		name:     "custom_doh_client_id",
		wantCode: http.StatusOK,
	}, {
		req:      httptest.NewRequest(http.MethodGet, "/dns-query", nil),
		name:     "legacy_doh_path_rejected",
		wantCode: http.StatusUnauthorized,
	}}
...
func generateAuthCookie(tb testing.TB, mux http.Handler, name, password string) (ac *http.Cookie) {
	return generateAuthCookieForPath(tb, mux, name, password, "/control/login")
}

// generateAuthCookieForPath is a helper function that logs in with the provided
// credentials using reqPath and returns the resulting authentication cookie.
func generateAuthCookieForPath(
	tb testing.TB,
	mux http.Handler,
	name string,
	password string,
	...
	r := httptest.NewRequest(http.MethodPost, reqPath, bytes.NewReader(creds))
...
func TestAuth_ServeHTTP_panelSessionCookiePath(t *testing.T) {
	const (
		testTTL = 60
	...
	loginReq := httptest.NewRequest(http.MethodPost, "/control/login", bytes.NewReader(creds))
	loginReq.Header.Set(httphdr.ContentType, aghhttp.HdrValApplicationJSON)
	loginReq = loginReq.WithContext(withPanelRequestContext(loginReq.Context(), panelPath+"/control/login", panelPath))
	...
	require.Equal(t, panelPath, loginCookie.Path)

	w := httptest.NewRecorder()
	r := httptest.NewRequest(http.MethodGet, "/control/logout", nil)
	r = r.WithContext(withPanelRequestContext(r.Context(), panelPath+"/control/logout", panelPath))
	r.AddCookie(loginCookie)
	mux.ServeHTTP(w, r)

	require.Equal(t, http.StatusFound, w.Code)

	gotExpired := w.Result().Cookies()
	require.NotEmpty(t, gotExpired)
	assert.Equal(t, panelPath, gotExpired[0].Path)
}
