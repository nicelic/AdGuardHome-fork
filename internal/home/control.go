...
func (web *webAPI) registerControlHandlers() {
	...
	web.registerCertificateManagerHandlers()
...
func (web *webAPI) handleHTTPSRedirect(w http.ResponseWriter, r *http.Request) (proceed bool) {
	...
	portHTTPS = effectivePanelServerPort(tlsConf)
	forceHTTPS = tlsConf.ForceHTTPS && tlsConf.Enabled && portHTTPS != 0
	...
	if forceHTTPS {
		if r.TLS == nil {
			reqURL := *r.URL
			reqURL.Path = originalPanelRequestPath(r)
			reqURL.RawPath = reqURL.Path
			u := httpsURL(&reqURL, host, portHTTPS)
...
func (web *webAPI) postInstallHandler(handler http.Handler) (wrapped http.Handler) {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if web.conf.firstRun &&
			...
			http.Redirect(w, r, panelURLForRequest(r, "/install.html"), http.StatusFound)
