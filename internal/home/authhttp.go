...
func (web *webAPI) handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	cookie, err := newCookie(ctx, web.auth, req, remoteIPStr, panelSessionCookiePath(r))
	if err != nil {
...
func (web *webAPI) handleLogout(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		respHdr.Set(httphdr.Location, panelURLForRequest(r, "/login.html"))
	...
	c = expiredSessionCookie(panelSessionCookiePath(r))
...
// panelSessionCookiePath returns the request-scoped cookie path for the panel
// session.  It follows the normalized panel prefix and defaults to the root.
func panelSessionCookiePath(r *http.Request) (path string) {
	path = panelRequestPrefix(r)
	if path == "" {
		return "/"
	}
...
// expiredSessionCookie returns an expired session cookie.
func expiredSessionCookie(path string) (c *http.Cookie) {
	return &http.Cookie{
		Name:    sessionCookieName,
		Value:   "",
		Path:    path,
...
func (web *webAPI) registerAuthHandlers() {
	...
	web.httpReg.Register(http.MethodPut, "/control/admin/update", web.handleAdminUpdate)
}
...
func (mw *authMiddlewareDefault) isDoHRoute(r *http.Request) (ok bool) {
	...
	if tlsConf.Enabled {
		return shouldServeDoHOnPanelWeb(tlsConf) && matchesCurrentDoHPath(r.URL.Path, tlsConf)
	}
...
func (mw *authMiddlewareDefault) handleAuthenticatedUser(
	...
	if path == "/login.html" {
		http.Redirect(w, r, panelURLForRequest(r, "/"), http.StatusFound)
...
func (mw *authMiddlewareDefault) handlePublicAccess(
	...
	if path == "/" || path == "/index.html" {
		http.Redirect(w, r, panelURLForRequest(r, "/login.html"), http.StatusFound)
