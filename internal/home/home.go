...
import (
	...
	"github.com/nicelic/AdGuardHome-fork/internal/arpdb"
	...
	"github.com/nicelic/AdGuardHome-fork/internal/filtering/hashprefix"
	"github.com/nicelic/AdGuardHome-fork/internal/filtering/safesearch"
	"github.com/nicelic/AdGuardHome-fork/internal/permcheck"
...
func initTLS(
	...
	tlsMgr, err = newTLSManager(ctx, &tlsManagerConfig{
		...
		manager:       aghtlsMgr,
		httpReg:       httpReg,
		certDir:       filepath.Join(confModifier.workDir, dataDir, "cert"),
...
https://github.com/nicelic/AdGuardHome-fork/wiki/Getting-Started#running-without-superuser`
...
func printWebAddrs(ctx context.Context, l *slog.Logger, proto, addr string, port uint16, reqPath string) {
	u := &url.URL{
		Scheme: proto,
...
// [urlutil.SchemeHTTP].  l must not be nil.
//
...
func printHTTPAddresses(ctx context.Context, l *slog.Logger, proto string, tlsMgr *tlsManager) {
	extTLSConf := currentTLSSettings(tlsMgr)

	port := config.HTTPConfig.Address.Port()
	if proto == urlutil.SchemeHTTPS {
		port = effectivePanelServerPort(extTLSConf)
	}

	reqPath := ""
	if panelPath := effectivePanelServerURLPath(extTLSConf); panelPath != defaultPanelServerURLPath {
		reqPath = panelPath + "/"
	}

	panelServerName := advertisedPanelServerName(extTLSConf)
	if proto == urlutil.SchemeHTTPS && panelServerName != "" {
		printWebAddrs(ctx, l, proto, panelServerName, port, reqPath)
	...
	if !bindHost.IsUnspecified() {
		printWebAddrs(ctx, l, proto, bindHost.String(), port, reqPath)
	...
	for _, iface := range ifaces {
		for _, addr := range iface.Addresses {
			printWebAddrs(ctx, l, proto, addr.String(), port, reqPath)
		}
	}
}
