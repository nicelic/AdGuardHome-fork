...
type SessionStorage interface {
	...
	// DeleteAll removes all stored web user sessions.
...
// DeleteAll implements the [SessionStorage] interface for
// *DefaultSessionStorage.
func (ds *DefaultSessionStorage) DeleteAll(ctx context.Context) (err error) {
	ds.mu.Lock()
	defer ds.mu.Unlock()

	err = ds.clear(ctx)
	if err != nil {
		ds.logger.ErrorContext(ctx, "deleting all sessions", slogutil.KeyError, err)
...
// clear deletes all web user sessions from the bbolt database.
func (ds *DefaultSessionStorage) clear(ctx context.Context) (err error) {
	tx, err := ds.db.Begin(true)
	if err != nil {
		return fmt.Errorf("starting transaction: %w", err)
	}
	...
	err = tx.DeleteBucket([]byte(bboltBucketSessions))
	if err != nil {
	...
	ds.logger.DebugContext(ctx, "removed all sessions from db")
