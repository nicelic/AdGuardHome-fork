...
type DB interface {
	...
	// Update updates an existing user in the database.  If another user already
	// has the same login, it returns the [errors.ErrDuplicated] error.  u must
	// not be modified.
...
// Update implements the [DB] interface for *DefaultDB.
func (db *DefaultDB) Update(ctx context.Context, u *User) (err error) {
	db.mu.Lock()
	defer db.mu.Unlock()
	...
	prev, ok := db.userIDToUser[u.ID]
	if !ok {
		return fmt.Errorf("userid: %w", errors.ErrNoValue)
	}

	if u.Login == "" {
		return fmt.Errorf("login: %w", errors.ErrEmptyValue)
	}

	id, ok := db.loginToUserID[u.Login]
	if ok && id != u.ID {
		return fmt.Errorf("login: %w", errors.ErrDuplicated)
	}

	delete(db.loginToUserID, prev.Login)
	db.userIDToUser[u.ID] = u
	db.loginToUserID[u.Login] = u.ID
