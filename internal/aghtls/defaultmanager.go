...
func (mgr *DefaultManager) Set(ctx context.Context, certKeys []TLSPair) (err error) {
	mgr.logger.DebugContext(ctx, "setting", "pairs_num", len(certKeys))
	...
	unwatch := map[string]string{}
	for _, old := range mgr.pairs {
		if old.CertPath != "" {
			unwatch[old.CertPath] = "old cert"
		}
		if old.KeyPath != "" {
			unwatch[old.KeyPath] = "old key"
		}
	}

	for path, what := range unwatch {
		errs = mgr.appendUnwatchErr(errs, what, path)
	}

	watch := map[string]string{}
	for _, certKey := range certKeys {
		if certKey.CertPath != "" {
			watch[certKey.CertPath] = "new cert"
		}
		if certKey.KeyPath != "" {
			watch[certKey.KeyPath] = "new key"
		}
	}

	for path, what := range watch {
		errs = mgr.appendWatchErr(errs, what, path)
	}

	mgr.pairs = append(mgr.pairs[:0], certKeys...)
